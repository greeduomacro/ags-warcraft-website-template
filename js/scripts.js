...
$(function(){
	...
	Body.find('#sidebars').each(function(index, Element){
		loadcss(site_uri("css/frontpage.css"));
		head.js(site_uri("js/AGS_Sidebars.js"), function(){			
			$(Element).AGS_Sidebars();
		});			
	});

	Body.find('.datatable').each(function(index, Element){
		loadcss(site_uri("css/jquery/datatables.css"));
		head.js(site_uri('js/jquery/datatable.js'), function(){
			$(Element).dataTable();
		});
	});
		
	Body.find( ".progressbar" ).each(function(index, Element){
		head.js('/js/jquery/ui/jquery.ui.progressbar.min.js', function(){
	...
	Body.find('a[data-wow], span[data-wow]').each(function(){
		head.js(site_uri('js/AGS_Wowtip.js'), function(){
			$(Element).AGS_Wowtip();
		});
	});
