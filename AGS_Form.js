...
;(function ( $, window, document, undefined ) {
    $.widget( "custom.AGS_Form" , {
        ...
        options: {
			...
			smileyPath: 		site_uri('images/smileys/'),
			captcha_url:		site_uri('captcha.php'),
			success: 			null
        }, 
		...
		_loadScripts: function(callback){			
			...
			if(this.options.style.radio || this.options.style.select || this.options.style.checkbox)
				head.js("/js/jquery/jstyling.js");
			
			head.js("/js/AGS_Tooltip.js");
			head.ready(callback);
		},
		...
		_bindElements: function(){
			...
			this.element.find('button[type="submit"]').each(function(index, Element){
				head.js('/js/jquery/ui/jquery.ui.button.min.js', function(){
					$(Element).button();
				});						
			}); //.button();
			...
			this.element.find('div.captcha').each(function(index, Element){
				head.js(
					site_uri("js/jquery/ui/jquery.ui.draggable.min.js"),
					site_uri("js/jquery/ui/jquery.ui.droppable.min.js"),
					site_uri("js/jquery/captcha.js"),
					function(){
			...
			this.element.find('textarea.bbcode').each(function(index, Element){
				head.js(site_uri('js/AGS_BBCode.js'), function(){
					$(Element).AGS_BBCode();				
				});
			});
			...
			if(realms.length > 0)
			{
				head.js(site_uri('js/jquery/ui/jquery.ui.autocomplete.min.js'), function(){				
					$.ajax({
			...
			// Date
			this.element.find('input[type="date"]').each(function(index, Element){
				head.js(site_uri('js/jquery/ui/jquery.ui.datepicker.min.js'), function(){
					that.date(Element);
				});
			});
			// Date time
			...
			// Date
			this.element.find('input[type="location"]').each(function(index, Element){
				head.js(site_uri('js/jquery/ui/jquery.ui.autocomplete.min.js'), function(){				
			...
			this.element.find('textarea.smileys').each(function(index, Element){
				head.js(site_uri('js/jquery/ui/jquery.ui.slider.min.js'), site_uri("js/jquery/insertatcarat.js"), function(){
		...
		_captcha: function(element){
			...
			var that = this;
			element.captcha({
				captchaDir: 	"images/jquery/captcha/",  
		...
		_validateForm: function(){
			...
			this.element.find('input[type=date]').rules("add", {
				 required: true,
				 date: true
			});
		...
		textareaSmileys: function( element ) {
			...
			$.ajax({
				url: 'exampledata.php?what=smileys',
				method: 'get',
				type: 'json',
				success: function(data){
